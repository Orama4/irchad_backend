...
import { getZones, getZonesCount, getZoneCountByDate } from '../services/zoneService';
...
export const getAllZones = async (req: Request, res: Response) => {
    try {
        ...
        const { zones, total } = await getZones(page, pageSize);
        res.status(200).json({
            zones,
            total,
...
export const getTotalZones = async (req: Request, res: Response) => {
    try {
      const { total } = await getZonesCount();
